...
export const useAppearanceSettingsStore = defineStore(
    ...
    () => {
        ...
        const profileCardOpacity = ref(0.8);
        ...
        async function initAppearanceSettings() {
            ...
            const [
                ...
                configRepository.getFloat('VRCX_profileCardOpacity', 0.8),
            ...
            profileCardOpacity.value = profileCardOpacityConfig;
         ...
         *
         */
        function setProfileCardOpacity(value) {
            profileCardOpacity.value = value;
            configRepository.setFloat('VRCX_profileCardOpacity', value);
        }

        // Card opacity drives the alpha of --profile-card (see globals.css).
        watch(
            profileCardOpacity,
            (opacity) => {
                document.documentElement.style.setProperty('--profile-card-alpha', `${Math.round(opacity * 100)}%`);
            },
            { immediate: true }
        );
